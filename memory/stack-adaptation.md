...
**More React-19/fiber-v9 patterns (confirmed in Prompt 03 — OrganismCanvas/context/hooks):**
- A THREE object that must be read in render (for `<primitive>`) AND mutated per-frame: prefer the **declarative element with a ref** (`<shaderMaterial ref={matRef} uniforms={memoizedUniforms} .../>`) and mutate `matRef.current.uniforms.x.value` in `useFrame`. Avoids both `react-hooks/refs` (don't read `.current` in render) and `react-hooks/immutability` (don't mutate useMemo/useState returns). Refs are exempt.
- Build imperative geometry in `useMemo` (mutating the *local* before returning is allowed), NOT a ref set in `useEffect` (a ref set triggers no re-render, so the mesh never appears) and NOT `setState` in an effect (lint error). Dispose via `useEffect(() => () => geo.dispose(), [geo])` (calling `.dispose()` in cleanup is allowed; R3F does not auto-dispose imperatively-passed geometry).
- Plain mutable holder objects (`const x = { current: 0 }` for throttles, `{ isPinching }` for gestures) are flagged by `react-hooks/immutability` ("cannot modify local variables after render"). Use `useRef` instead.
- `Date.now()` / other impure calls **directly** in render (e.g. `useState(Date.now())`) → `react-hooks/purity` error. Wrap in a lazy initializer: `useState(() => Date.now())`.
- Move per-render-recreated config objects used inside `useCallback`/`useMemo` to **module scope** to avoid `exhaustive-deps` warnings.
- **GLSL**: `THREE.ShaderMaterial` emits **GLSL ES 1.00** — `transpose()` and `inverse()` are GLSL ES 3.00-only and will fail to compile at runtime. For a uniformly-scaled mesh use `mat3(modelMatrix) * normal` for the world normal (normalize absorbs scale). THREE auto-injects `position/normal/uv/modelMatrix/modelViewMatrix/projectionMatrix/cameraPosition/normalMatrix` for ShaderMaterial — do not redeclare them. (Note: GLSL compile errors only surface at runtime in a browser, never in `next build`.)
